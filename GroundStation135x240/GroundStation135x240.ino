// Uncomment #include <User_Setups/Setup25_TTGO_T_Display.h> in ./libraries/TFT_eSPI/User_Setup_Select.h to make display working.
#include <TFT_eSPI.h>
#include <SPI.h>
#include <esp_now.h>
#include "WiFi.h"
...
void infoRender(char *info, uint32_t value, uint32_t x, uint32_t y)
{
  tft.setTextSize(1);
  tft.setTextColor(TFT_GREEN);
...
{
  uint32_t x_offset = 164;
  uint32_t y_offset = 0;
  uint32_t y_step = 35;
  uint32_t x_size = 76;
  uint32_t y_size = 135;

  tft.setAddrWindow(x_offset, 0, x_size, y_size);
  for (uint32_t i = 0; i < x_size * y_size; i++)
  {
    tft.pushColor(TFT_BLACK);
  }

  infoRender("Lat:", tele.lat, x_offset, y_step * y_offset++);
  infoRender("Lon:", tele.lon, x_offset, y_step * y_offset++);
  infoRender("Alt:", tele.alt, x_offset, y_step * y_offset++);
  infoRender("Temp:", tele.temp, x_offset, y_step * y_offset++);
}

void packetsRender(uint32_t last_packet_time)
{
  uint32_t max_line_width = 160;
  uint32_t line_height = 5;
